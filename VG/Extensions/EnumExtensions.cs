...
namespace VG.Extensions
{
    ...
    public static class EnumExtensions
    {
        ...
        {
            TEnum[] l_enumValues = (TEnum[]) Enum.GetValues (typeof (TEnum));

            if (string.IsNullOrWhiteSpace (value) == false)
            {
                string l_value = value.ToLowerInvariant ();

                foreach (TEnum i_enumValue in l_enumValues)
                {
                    string i_enumAsString = i_enumValue.ToString ().ToLowerInvariant ();
        ...
        public static TEnum ToEnum<TEnum> (this string value, TEnum p_defaultValue) where TEnum : Enum
        {
            if (string.IsNullOrWhiteSpace (value) == false)
            {
                ...
                foreach (TEnum i_enumValue in (TEnum[]) Enum.GetValues (typeof (TEnum)))
                {
                    string i_enumAsString = i_enumValue.ToString ().ToLowerInvariant ();
        ...
        /// <param name="p_setAsEverythingIfEmpty">Установить Enum, как Everything (все флаги), если массив пуст (false -> Nothing)?</param>
        public static TEnum ToFlagsEnum<TEnum> (this string[] value, bool p_setAsEverythingIfEmpty = false) where TEnum : Enum
        {
            TEnum[] l_enumValues = (TEnum[]) Enum.GetValues (typeof (TEnum));
            ...
            if (value != null && value.Length > 0)
            {
                foreach (var i_string in value)
                {
                    ...
                    {
                        long i_enumValueLong = (long) Convert.ChangeType (i_enumValue, TypeCode.Int64);
