...
namespace VG.Extensions
{
    ...
    public static class ListExtensions
    {
        ...
        public static IEnumerable<T> Clone<T> (this IEnumerable<T> original) where T : ICloneable
        {
            ...
            if (original != null)
            {
                T[] array = original as T[] ?? original.ToArray ();
        ...
        public static int DishonestRandom (this IEnumerable<int> chances, Random random = null) =>
            DishonestRandom (chances?.Select (chance => (double) chance), random);
        ...
        public static int DishonestRandom (this IEnumerable<double> chances, Random random = null)
        {
            if (chances != null)
            {
                double[] chancesAsArray = chances as double[] ?? chances.ToArray ();
                double chancesSum = chancesAsArray.Sum (chance => Math.Max (0, chance));

                double randomValue = random.NextDouble (0, chancesSum);
                double checkedSum = 0;

                for (int chanceIndex = 0; chanceIndex < chancesAsArray.Length; chanceIndex++)
                {
                    double chance = chancesAsArray[chanceIndex];
        ...
        public static List<T0> Intersect<T0, T1> (this IEnumerable<T0> first, IEnumerable<T1> second, Func<T0, T1, bool> func)
        {
