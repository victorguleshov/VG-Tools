    ...
    public class Math3d
    {
        ...
        public static float[] GetLineSplineIntersections (Vector2[] linePoints, Vector2[] cPoints)
        {
            ...
            for (var i = 0; i < numSections; i++)
            {
                ...
                var a1 = 0.5f * (-p0.x + 3f * p1.x - 3f * p2.x + p3.x);
                var a2 = 0.5f * (-p0.y + 3f * p1.y - 3f * p2.y + p3.y);
                var b1 = 0.5f * (2f * p0.x - 5f * p1.x + 4f * p2.x - p3.x);
                var b2 = 0.5f * (2f * p0.y - 5f * p1.y + 4f * p2.y - p3.y);
                var c1 = 0.5f * (-p0.x + p2.x);
                var c2 = 0.5f * (-p0.y + p2.y);
                var d1 = 0.5f * (2f * p1.x);
                var d2 = 0.5f * (2f * p1.y);
        ...
        private static void SolveCubic (out int nRoots, out float x1, out float x2, out float x3, float a, float b, float c, float d)
        {
            ...
            var Q = (3f * b - a * a) / 9f;
            var Q_CUBE = Q * Q * Q;
            var R = (9f * a * b - 27f * c - 2f * a * a * a) / 54f;
            var R_SQR = R * R;
            var D = Q_CUBE + R_SQR;
        ...
        public static bool LinearAcceleration (out Vector3 vector, Vector3 position, int samples)
        {
            ...
            for (var i = 0; i < positionRegister.Length - 1; i++)
            {
            ...
            if (positionSamplesTaken >= samples)
            {
                ...
                for (var i = 0; i < positionRegister.Length - 2; i++)
                {
                ...
                var deltaTimeTotal = posTimeRegister[posTimeRegister.Length - 1] - posTimeRegister[0];
        ...
        public static bool AngularAcceleration (out Vector3 vector, Quaternion rotation, int samples)
        {
            ...
            // each time a new sample is taken. This way index 0 will always hold the oldest sample and the
            // highest index will always hold the newest sample. 
            for (var i = 0; i < rotationRegister.Length - 1; i++)
            {
            ...
            if (rotationSamplesTaken >= samples)
            {
                ...
                // Get the total time difference.
                var deltaTimeTotal = rotTimeRegister[rotTimeRegister.Length - 1] - rotTimeRegister[0];
